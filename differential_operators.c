...
void udxdy (double *u_out, double *u_in, int N_x, int N_y, double h_x, double h_y, double L_x, double L_y) {
    ...
    for (j = 1; j < N_x; ++j) {
        u_out[j] = (u_in[(1)*(N_x+1)+j+1]-u_in[(1)*(N_x+1)+j-1]-u_in[j+1]+u_in[j-1])/(h_x*h_y);
        u_out[(N_y-1)*(N_x+1)+j] = (u_in[(N_y-1)*(N_x+1)+j+1]-u_in[(N_y-1)*(N_x+1)+j-1]-u_in[(N_y-2)*(N_x+1)+j+1]+u_in[(N_y-2)*(N_x+1)+j-1])/(h_x*h_y);
    }
...
void vdxdy (double *v_out, double *v_in, int N_x, int N_y, double h_x, double h_y, double L_x, double L_y) {
    ...
    for (i = 1; i < N_y; ++i) {
        v_out[i*N_x+0] = (v_in[(i+1)*N_x+1]-v_in[(i+1)*N_x]-v_in[(i-1)*N_x+1]+v_in[(i-1)*N_x])/(h_x*h_y);
        v_out[i*N_x+N_x-1] = (v_in[(i+1)*N_x+N_x-1]-v_in[(i+1)*N_x+N_x-2]-v_in[(i-1)*N_x+N_x-1]+v_in[(i-1)*N_x+N_x-2])/(h_x*h_y);
    }
